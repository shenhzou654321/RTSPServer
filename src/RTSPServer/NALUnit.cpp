...
bool NALUnit::GetStartCode(const BYTE *&pBegin, const BYTE *&pStart, int &cRemain)
{
...
bool NALUnit::Parse(const BYTE *pBuffer, int cSpace, int LengthSize, bool bEnd)
{
...
unsigned long NALUnit::GetWord(int nBits)
{
    unsigned long u = 0;
    while (nBits > 0)
    {
...
void ScalingList(int size, NALUnit *pnalu)
{
    long lastScale = 8;
    long nextScale = 8;
...
bool SeqParamSet::Parse(NALUnit *pnalu)
{
...
// --- slice header --------------------
bool SliceHeader::Parse(NALUnit *pnalu)
{
    switch(pnalu->Type())
    {
